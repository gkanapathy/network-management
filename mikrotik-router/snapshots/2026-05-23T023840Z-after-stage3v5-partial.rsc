# 2026-05-22 19:38:42 by RouterOS 7.21.4
...
add check-gateway=ping distance=1 dst-address=0.0.0.0/0 gateway=\
    162.217.74.129 routing-table=main
add distance=2 dst-address=0.0.0.0/0 gateway=23.93.120.1 routing-table=main
add check-gateway=ping distance=1 dst-address=0.0.0.0/0 gateway=\
    162.217.74.129 routing-table=mb
add distance=2 dst-address=0.0.0.0/0 gateway=23.93.120.1 routing-table=mb
add check-gateway=ping distance=1 dst-address=0.0.0.0/0 gateway=23.93.120.1 \
    routing-table=sonic
add distance=2 dst-address=0.0.0.0/0 gateway=162.217.74.129 routing-table=\
    sonic
/ipv6 route
add check-gateway=ping distance=1 dst-address=::/0 gateway=\
    fe80::f61e:57ff:fe09:94ab%ether2 routing-table=main
add distance=2 dst-address=::/0 gateway=\
    fe80::5e5e:abff:feda:ebc0%sfp-sfpplus1 routing-table=main
add check-gateway=ping distance=1 dst-address=::/0 gateway=\
    fe80::f61e:57ff:fe09:94ab%ether2 routing-table=mb
add distance=2 dst-address=::/0 gateway=\
    fe80::5e5e:abff:feda:ebc0%sfp-sfpplus1 routing-table=mb
add check-gateway=ping distance=1 dst-address=::/0 gateway=\
    fe80::5e5e:abff:feda:ebc0%sfp-sfpplus1 routing-table=sonic
add distance=2 dst-address=::/0 gateway=fe80::f61e:57ff:fe09:94ab%ether2 \
    routing-table=sonic
/ip service
...
set host-key-size=4096 host-key-type=ed25519 password-authentication=yes \
    strong-crypto=yes
/ipv6 address
add address=fd7f:aee1:6ce0:88::1 interface=vlan88
add address=fd7f:aee1:6ce0:10::1 interface=vlan10
add address=fd7f:aee1:6ce0:20::1 interface=vlan20
add address=fd7f:aee1:6ce0:30::1 interface=vlan30
add from-pool=mb-pd interface=vlan88
add address=0:0:0:1:: from-pool=mb-pd interface=vlan10
add address=0:0:0:2:: from-pool=mb-pd interface=vlan20
add address=0:0:0:3:: from-pool=mb-pd interface=vlan30
add from-pool=sonic-pd interface=vlan88
add address=0:0:0:1:: from-pool=sonic-pd interface=vlan10
add address=0:0:0:2:: from-pool=sonic-pd interface=vlan20
add address=0:0:0:3:: from-pool=sonic-pd interface=vlan30
/ipv6 dhcp-client
add interface=ether2 pool-name=mb-pd request=address,prefix
add interface=sfp-sfpplus1 pool-name=sonic-pd request=address,prefix
...
add advertise-dns=yes dns=fd7f:aee1:6ce0:88::1 interface=vlan88
add advertise-dns=yes dns=fd7f:aee1:6ce0:10::1 interface=vlan10
add advertise-dns=yes dns=fd7f:aee1:6ce0:20::1 interface=vlan20
add advertise-dns=yes dns=fd7f:aee1:6ce0:30::1 interface=vlan30
/routing rule
add action=lookup comment=\
    "LAN dsts -> main (catches reply + inter-VLAN before src rules)" \
    dst-address=192.168.0.0/16 table=main
add action=lookup comment="plumtree -> sonic" src-address=192.168.10.0/24 \
    table=sonic
add action=lookup comment="guest -> mb" src-address=192.168.20.0/24 table=mb
add action=lookup comment="iot -> mb" src-address=192.168.30.0/24 table=mb
add action=lookup comment="mgmt -> mb" src-address=192.168.88.0/24 table=mb
add action=lookup comment="v6 ULA LAN dsts -> main" dst-address=\
    fd7f:aee1:6ce0::/48 table=main
add action=lookup comment="v6 MB-pd LAN dsts -> main" dst-address=\
    2607:f598:d488:6100::/56 table=main
add action=lookup comment=\
    "v6 Sonic-pd LAN dsts -> main (UPDATE if PD rotates)" dst-address=\
    2001:5a8:6a5:4600::/56 table=main
add action=lookup comment="v6 MB-pd src -> mb" src-address=\
    2607:f598:d488:6100::/56 table=mb
add action=lookup comment="v6 Sonic-pd src -> sonic (UPDATE if PD rotates)" \
    src-address=2001:5a8:6a5:4600::/56 table=sonic
/system clock
...
/system script
add dont-require-permissions=no name=toggle-leds owner=*sys policy=\
